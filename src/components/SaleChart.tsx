...
const SaleChart = ({ sales }: Props) => {
  const theme = useTheme();

  const N = 10; // cantidad de ventas recientes a mostrar
  ...
  // Toma las últimas N ventas (más recientes)
  const recentSales = sortedSales.slice(0, N);

  // Agrupa la cantidad total por producto SOLO en esas ventas recientes
  const grouped = recentSales.reduce<Record<string, number>>(
    (acc, curr) => {
  ...
  return (
    ...
    >
      ...
      >
        Cantidad de productos vendidos (últimas {N} ventas)
      </Typography>
      <Box
        sx={{
